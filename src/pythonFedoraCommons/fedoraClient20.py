...
import Fedora_2_0_API_A_Service_services
import Fedora_2_0_API_M_Service_services
...
class FedoraClient(object):
    def __init__(self, serverurl='http://localhost:8080/fedora', username='fedoraAdmin', password='fedoraAdmin', version="2.2"):
        ...
        access_tail = '/access/soap'
        management_tail = '/management/soap'

        # Setup API-A SOAP proxy
        api_a_kw={'auth' : (ZSI.auth.AUTH.httpbasic, username, password), 'url' : self.server + access_tail}
        api_a_loc = Fedora_2_0_API_A_Service_services.Fedora_API_A_ServiceLocator()
        ...
        api_m_loc = Fedora_2_0_API_M_Service_services.Fedora_API_M_ServiceLocator()
    ...
    def getObjectProfile_SOAP(self, pid):
        request = Fedora_2_0_API_A_Service_services.getObjectProfileRequest()
    ...
    def getDatastreamProfile(self, id, dsid, asOfDateTime=None):
        ...
        request = Fedora_2_0_API_M_Service_services.getDatastreamRequest()
        request._pid = id
        request._dsID = dsid
        if asOfDateTime:
    ...
    def modifyDSByValue(self, id, dsid, altids, dsLabel, mimeType, formatURI, dsContent, checksumType, checksum, logMessage, force):
        ...
        request = Fedora_2_0_API_M_Service_services.modifyDatastreamByValueRequest()
        request._pid = id
    ...
    def addDatastream(self, id, dsid, altids, dsLabel, versionable, mimeType, formatURI, dsLocation, controlGroup, dsState, checksumType, checksum, logMessage):
        ...
        request = Fedora_2_0_API_M_Service_services.addDatastreamRequest()
        request._pid = id
        request._MIMEType = mimeType
        request._altIDs = altids
        request._checksum = checksum
    ...
    def addString(self, id, dsid, dsLabel, versionable, XMLstring, controlGroup, dsState, logMessage, temporary_store, mimetype="text/xml"):
        ext = u'.xml'
    ...
    def modifyDSByReference(self, id, dsid, altids, dsLabel, mimeType, formatURI, dsLocation, checksumType, checksum, logMessage, force):
        ...
        request = Fedora_2_0_API_M_Service_services.modifyDatastreamByReferenceRequest()
        request._pid = id
        request._MIMEType = mimeType
        request._altIDs = altids
        request._checksum = checksum
    ...
    def getNextPID(self, namespace=u"ora"):
        request = Fedora_2_0_API_M_Service_services.getNextPIDRequest()
    ...
    def ingest(self, objectXML, logMessage='Ingest object', format='foxml1.0'):
        request = Fedora_2_0_API_M_Service_services.ingestRequest()
    ...
    def purgeDatastream(self, pid, dsid, logMessage='deleted datastream'):
        request = Fedora_2_0_API_M_Service_services.purgeDatastreamRequest()
        request._pid = pid
        request._logMessage = logMessage
        request._dsID = dsid
    ...
    def purgeObject(self, pid, logMessage='deleted object'):
        request = Fedora_2_0_API_M_Service_services.purgeObjectRequest()
        request._pid = pid
        request._logMessage = logMessage
        request._force = False
    ...
    def modifyObject(self, pid, label, ownerId, logMessage='Modified object', state='A' ):
        request = Fedora_2_0_API_M_Service_services.modifyObjectRequest()
        request._pid = pid
    ...
    def replace(self, id, dsid, filename, label, logMessage):
        ...
        dsLocation = loc
        altids = FedoraAPI_2_0_ns.ArrayOfString_Def('').pyclass
        dsLabel = label
    ...
    def getBlankAltIDs(self):
        return FedoraAPI_2_0_ns.ArrayOfString_Def('').pyclass    
    ...
    def doesObjectExist_SOAP(self, pid):
        ...
        request = Fedora_2_0_API_M_Service_services.getDatastreamsRequest()
        request._pid =  pid
